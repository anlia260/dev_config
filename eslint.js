...
// This pattern will match these texts:   var Foo = require('Foo');   var Bar =
// require('Foo').Bar;   var BarFoo = require(Bar + 'Foo');   var {Bar, Foo} =
// require('Foo');   import type {Bar, Foo} from 'Foo'; Also supports 'let' and
// 'const'.
const variableNamePattern = String.raw `\s*[a-zA-Z_$][a-zA-Z_$\d]*\s*`;
const maxLenIgnorePattern = String.raw `^(?:var|let|const|import type)\s+` + '{?' + variableNamePattern + '(?:,' + variableNamePattern + ')*}?' + String.raw `\s*(?:=\s*require\(|from)[a-zA-Z_+./"'\s\d\-]+\)?[^;\n]*[;\n]`;
...
function getBaseConfig() {
    return {
        ...
        //
        // If you update eslint, be sure to check the changelog to figure out what rules
        // to add/remove to/from this list.
        rules: {
            // Possible Errors <http://eslint.org/docs/rules/#possible-errors> Forked and
            // moved to fb-www/comma-dangle
            'comma-dangle': OFF,
            ...
            // Best Practices <http://eslint.org/docs/rules/#best-practices> probably a bug,
            // we shouldn't actually even use this yet, because of IE8
            'accessor-pairs': [
                WARNING, {
                    setWithoutGet: true
                }
            ...
            'no-iterator': OFF,
            // Should be effectively equivalent to jshint W028 - allowing the use of labels
            // in very specific situations. ESLint no-empty-labels was deprecated.
            'no-labels': [
                ERROR, {
                    allowLoop: true,
                    allowSwitch: true
            ...
            // Strict Mode <http://eslint.org/docs/rules/#strict-mode> jshint wasn't
            // checking this, and the compiler should add this anyways
            'strict': OFF,

            // Variables <http://eslint.org/docs/rules/#variables> we don't do this/care
            // about this
            ...
            // Stylistic Issues <http://eslint.org/docs/rules/#stylistic-issues> See also:
            // https://our.intern.facebook.com/intern/dex/style-guide/
            ...
            // TODO: Figure out a way to do this that doesn't break typechecks or wait for
            // https://github.com/eslint/eslint/issues/2897
            'spaced-comment': [
                OFF,
                'always', {
        ...
        globals: {
            ...
            requireDynamic: true,
            requireLazy: true,
            // more haste variables are defined in getConfig for modules Workarounds for
            // https://github.com/babel/babel-eslint/issues/130 no-undef errors incorrectly
            // on these global flow types https://fburl.com/flow-react-defs
            ...
            // a bunch of types extracted from http://git.io/vOtv9 there's a bunch of
            // overlap with browser globals, so we try to avoid redefining some of those.
            $Either: false,
            $All: false,
            $Tuple: false,
            $Supertype: false,
            ...
            // doesn't have proper WebRTC support yet https://fburl.com/flow-webrtc-defs
